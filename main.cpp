...
/*
    TEST_CASE("PGMimageProcessor Big 6 Unit Tests")
    {
        ...
        SECTION("Custom Constructor")
        {
            ...
            REQUIRE(p.getComment() == "# Created by GIMP version 2.10.18 PNM plug-in");
        ...
        SECTION("Copy Constructor")
        {
            ...
            REQUIRE(p2.getComment() == p1.getComment());
            REQUIRE(p2.getHeight() == p1.getHeight());
            REQUIRE(p2.getWidth() == p1.getWidth());
            REQUIRE(p2.getThreshold() == p1.getThreshold());

            unsigned char ** p1_array = p1.getArray();
            unsigned char ** p2_array = p2.getArray();
        ...
        {
            PGMimageProcessor p1("sloan_image.pgm");
            unsigned char **arrAdr = p1.getArray();

            PGMimageProcessor p2(std::move(p1));
            REQUIRE(p1.getVersion()==p2.getVersion());
            REQUIRE(p1.getComment()==p2.getComment());
     ...
     TEST_CASE("PGMimageProcessor Functions")
    {
        ...
        {
            PGMimageProcessor p1("tester.pgm");
            //valid size = 1
            int num = p1.extractComponents(45,1);
            REQUIRE(num==p1.getVector().size());
            //valid size = 3
            PGMimageProcessor p2("tester.pgm");
  ...
  TEST_CASE("ConnectedComponents Functions")
    {
        ...
        SECTION("Custom Constructor")
        {
            ...
            pair1.first = 0;
            pair1.second = 0;
            map<unsigned char, pair<int,int>> cmap;
            cmap[uc] = pair1;
            ConnectedComponent c(400,5000,cmap);
            REQUIRE(c.getID()==400);
            REQUIRE(c.getMap().size()==1);
            map<unsigned char, pair<int,int>> constr_map = c.getMap();
        ...
        SECTION("Copy Constructor")
        {
            ...
            map<unsigned char, pair<int,int>> constr1_map = c1.getMap();
            map<unsigned char, pair<int,int>> constr2_map = c2.getMap();
